...
std::unordered_map<std::string, boost::circular_buffer<std::shared_ptr<Sample>>> Source::sampleCloset;
std::mutex Source::mutex;
...
GstFlowReturn Source::on_new_sample(GstElement* appsink, gpointer data)  {
    ...
    std::lock_guard locker(Source::mutex);
    // Make circ. buffer of size 10 for frames
    if (!Source::sampleCloset.contains(s_uuid))
        Source::sampleCloset[s_uuid] = boost::circular_buffer<std::shared_ptr<Sample>>(10);
    
    Source::sampleCloset[s_uuid].push_back(std::make_shared<Sample>(sample));
...
Source::~Source() {
    ...
    Source::mutex.lock();
    Source::sampleCloset.erase(s_uuid);
    Source::mutex.unlock();
...
std::shared_ptr<Sample> Source::getSample() const {
    std::lock_guard locker(mutex);
    
    auto copy = Source::sampleCloset.at(boost::uuids::to_string(*uuid)).front();
    Source::sampleCloset.at(boost::uuids::to_string(*uuid)).pop_front();
    
    return copy;
}

void Source::waitSample() const {
    while (!Source::sampleCloset.contains(boost::uuids::to_string(*uuid)));
    while (!Source::sampleCloset.at(boost::uuids::to_string(*uuid)).size());
}